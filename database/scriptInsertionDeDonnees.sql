...
set identity_insert backend.src.main.java.com.meetPeople.SituationFamiliale on;
insert into SituationFamiliale(idSituationFamiliale, description) values
(1,'A des enfants'),
(2,'N''a pas d''enfants');
set identity_insert backend.src.main.java.com.meetPeople.SituationFamiliale off;

set identity_insert Interet on;
insert into Interet (idInteret, description) values
(1,'Relation serieuse'),
(2,'Amitie'),
(3,'Relation non serieuse'),
(4,'Marriage');
set identity_insert Interet off;

set identity_insert Admin on;
insert into Admin 
(numeroEmploye, courriel, nom, prenom, motDePasse) values
...
insert into Membre
(idMembre, courriel, nom, prenom, motDePasse, estActif, detailProfil, sexe, grandeur, dateDeNaissance, estVerifie, idSituationFamiliale, idInteret, numeroAdminDesactive) values
...
insert into Photo
(idPhoto, cheminURL, estPhotoProfil, idMembre) values
...
insert into Message
(idMessage, contenu, dateMessage, idMembreExpediteur, idMembreRecevant) values
...
(idEvaluation, note, description, date, idMembreEvaluateur, idMembreEvalue) values
(1,3,'Bonne personne mais pas mon style','2022-10-17',1,2);
set identity_insert Evaluation off;


insert into MatchTable 
(idMembreInitiateur, idMembreSecond, estAimerParInitiateur, estAimerParSecond, dateInitiateur, dateSecond) values
(1,2,1,1,'2022-09-15','2022-09-17'),
(1,5,1,1,'2022-09-12','2022-09-15'),
(3,6,1,1,'2022-10-11','2022-10-14');

set identity_insert Plainte on;
insert into Plainte
(idPlainte, contenu, date, numeroEmploye, idMembrePlaignant, idMembreConcerner) values
(1,'Personne agresive','2022-10-10',2,3,6);
set identity_insert Plainte off;
